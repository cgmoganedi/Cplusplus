...
void Texture::render(int x, int y, SDL_Rect src)
{
    ...
    // Call SDL_RenderCopy(?????)
    Main_Window = SDL_CreateWindow("Pacman", x, y, src.w, src.h, 0);
    Main_Renderer = SDL_CreateRenderer(Main_Window, -1, SDL_RENDERER_ACCELERATED);
    
    Loading_Surf = SDL_LoadBMP("sprites.png");
	myTexture = SDL_CreateTextureFromSurface(Main_Renderer, Loading_Surf);
	
	SDL_FreeSurface(Loading_Surf);
	
	SDL_RenderCopy(Main_Renderer, myTexture, NULL, NULL);
	SDL_RenderCopy(Main_Renderer, myTexture, &src, &dest);
