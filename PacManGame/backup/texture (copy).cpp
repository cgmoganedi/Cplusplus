...
void Texture::render(int x, int y, SDL_Rect src)
{
    ...
    gWindow = SDL_CreateWindow("Pacman", x, y, src.w, src.h, SDL_WINDOW_SHOWN);
    if (gWindow == nullptr){
    	...
    	if(gRenderer == nullptr){
    		...
    		if( !(IMG_Init(imgFlags) & imgFlags) ){
    			std::cout<<"SDL_image could not be initialises! SDL_image Error: %s\n"<<SDL_GetError()<<std::endl;
    			return;
    		}
			else{
		        //The final texture
		        ...
		        SDL_Surface* loadedSurface = IMG_Load( "sprites.png");
		        if( loadedSurface == nullptr ){
		                std::cout<<"Unable to load image %s! SDL_image Error: %s\n"<< "sprites.png"<<IMG_GetError();
		                return;
		        }
		        else{
		            //Create texture from surface pixels
		            myTexture = SDL_CreateTextureFromSurface( gRenderer, loadedSurface );
		            if( myTexture == nullptr ){
